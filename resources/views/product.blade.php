        ...
        <div class="container px-5 mx-auto">
            <form method="get" action="/" class="flex items-center space-x-5">
                ...
                <div class="flex items-center space-x-2">
                    <button type="submit"
                        class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Pesquisar</button>
                    <a href="/">Limpar</a>
                </div>
            </form>
        </div>
                ...
                @foreach ($products as $product)
                    <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
                        <a class="block relative h-48 rounded overflow-hidden">
                            <img alt="Foto produto" class="object-cover object-center w-full h-full block"
                                src="{{ $product->cover }}">
                            {{-- height="250px" --}}
                        ...
                        <div style="margin-left: 15%" class="mt-3 text-indigo-500 inline-flex items-center">
                        <add-to-cart-button product-id="{{$product->id}}"
                            user-id="{{auth()->user()->id ?? 0}}"
                        />
                        </div>
                        {{-- <a href="{{ route('product', $product->slug) }}"
                            class="mt-3 text-indigo-500 inline-flex items-center"> Carrinho
                            <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                stroke-width="2" class="w-4 h-4 ml-2" viewBox="0 0 24 24">
                                <path d="M5 12h14M12 5l7 7-7 7"></path>
                            </svg>
